...
async def collect_generic_offers(runtime: StagehandRuntime, board: BoardConfig, page: Page) -> DiscoveryResult:
    ...
    stagnation = 0
    repeated_idle_listing_steps = 0
    last_idle_snapshot: Optional[tuple[str, int, str, str, int]] = None
    step = 0
    ...
    while True:
        ...
        if not moved and not queued_pages and added_this_step == 0 and (raw_extracted or seen):
            idle_snapshot = (
                current_page,
                len(raw_extracted),
                raw_extracted[0] if raw_extracted else "",
                raw_extracted[-1] if raw_extracted else "",
                len(seen),
            )
            if idle_snapshot == last_idle_snapshot:
                repeated_idle_listing_steps += 1
            else:
                repeated_idle_listing_steps = 1
        ...
        if repeated_idle_listing_steps >= 2:
            console.print("STOP_REASON=repeated_idle_listing")
            break
