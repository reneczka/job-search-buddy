"""
Scrape Runner - Shared orchestration logic for job scraping.

This module contains all the common logic used by both job_boards_scraper.py
and career_sites_scraper.py, eliminating code duplication.
"""
...
def _parse_records(output_text: str) -> Optional[List[dict]]:
...
async def scrape_source(
    ...
    Args:
        agent: The Playwright agent instance
        source_record: Airtable record containing source URL
        column_name: Column name to extract URL from ("Job Boards" or "Career Sites")
        prompt_generator: Function to generate the scraping prompt
...
async def scrape_sources(
    ...
    Args:
        agent: The Playwright agent instance
        sources: List of Airtable source records
        column_name: Column name to filter and extract URLs from
        prompt_generator: Function to generate the scraping prompt
        
    Returns:
        Combined list of all scraped job records
    """
...
async def run_scraper(
    ...
    """Run the scraper for a specific source type.
    
    This is the main orchestration function that handles the entire scraping pipeline:
    - Environment validation
    - Source fetching (hardcoded or Airtable)
    - Playwright server and agent setup
    - Concurrent scraping
    - Airtable sync
    - Timing reports
    
    Args:
        scraper_name: Display name for the scraper (e.g., "Job Boards", "Career Sites")
        column_name: Airtable column name to use ("Job Boards" or "Career Sites")
        prompt_generator: Function to generate scraping prompts
        hardcoded_url: URL to use when USE_HARDCODED_SOURCE is True
        sort_by: Optional sort field for Airtable query
    """
    try:
        program_start_time = time.time()
        console.print(f'\n[bold blue]{scraper_name} scraper started at {datetime.now().strftime("%H:%M:%S")}[/]')
        ...
        if USE_HARDCODED_SOURCE:
            ...
            sources = [
                {
                    "fields": {
                        column_name: hardcoded_url,
                    }
                }
            ]
            client = None
            console.print(Panel(
                f"Using hardcoded {scraper_name.lower()} source.",
                title="Sources",
                style="blue",
            ))
        else:
            ...
            if not sources_table_id:
                ...
                return
            sources = client.get_all_records(sources_table_id, sort_by=sort_by)
            sources_fetch_end = time.time()
            sources_fetch_time = sources_fetch_end - sources_fetch_start
            console.print(Panel(
                f"Fetched {len(sources)} {scraper_name.lower()} sources from Airtable in {format_duration(sources_fetch_time)}.",
                title="Sources",
                style="blue"
            ))
            ...
            if not sources:
                console.print(Panel(
                    f"No {scraper_name.lower()} sources found in Airtable.",
                    title="Warning",
                    style="yellow",
                ))
                return
        ...
        console.print(f'\n[bold blue]Starting concurrent {scraper_name.lower()} scraping from {len(sources)} sources...[/]')
        
        async with create_playwright_server() as playwright_server:
            agent = create_playwright_agent(playwright_server)

            # Scrape all sources
            all_records = await scrape_sources(agent, sources, column_name, prompt_generator)
            ...
            console.print(f'\n[bold green]Total jobs scraped: {len(all_records)} from {len(sources)} sources[/]')
        ...
        console.print(f'\n[bold green]🎉 {scraper_name} scraper completed successfully in {format_duration(total_program_time)}[/]')
