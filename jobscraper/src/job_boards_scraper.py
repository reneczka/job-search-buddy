...
async def main() -> None:
    """Main application entry point for job boards scraper"""
    try:
        program_start_time = time.time()
        console.print(f'\n[bold blue]Job Boards scraper started at {datetime.now().strftime("%H:%M:%S")}[/]')
        ...
        if USE_HARDCODED_SOURCE:
            ...
            client = None
            console.print(Panel(
                "Using hardcoded job board source.",
                title="Sources",
                style="blue",
            ))
        else:
            ...
            console.print(Panel(f"Fetched {len(sources)} job board sources from Airtable in {format_duration(sources_fetch_time)}.", title="Sources", style="blue"))
            ...
            if not sources:
                console.print(Panel(
                    "No job board sources found in Airtable.",
                    title="Warning",
                    style="yellow",
                ))
                return
        ...
        console.print(f'\n[bold blue]Starting concurrent job board scraping from {len(sources)} sources...[/]')
        ...
        console.print(f'\n[bold green]🎉 Job Boards scraper completed successfully in {format_duration(total_program_time)}[/]')
...
async def scrape_source(agent: Any, source_record: dict) -> list:
    """Scrape jobs from a single job board source asynchronously with retry logic."""
    ...
    source_name = extract_source_name(source_url)
    console.print(f"[dim]Processing job board: {source_name} ({source_url})[/]")

    # Build task prompt
    task_prompt = generate_job_board_instructions(url=source_url, source_name=source_name)
