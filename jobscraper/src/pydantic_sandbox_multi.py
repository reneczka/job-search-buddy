...
def selected_targets(site: str) -> list[TargetBoard]:
    all_targets = [
    ...
    if site == "all":
        return all_targets
    return [target for target in all_targets if target.name == site]
...
def is_offer_candidate_url(url: str, domain: str) -> bool:
    ...
    if parts and parts[0] in blocked_segments:
        return False
    # Domain-aware final-offer URL gates for the two supported boards.
...
async def collect_offers(
    ...
    while True:
        ...
        if header_on_current is not None:
            ...
            if step == 1 and board_name != "justjoin":
        ...
        moved = False if board_name == "justjoin" else await reveal_more(session, pw_page, model_opts, model_name, cache_enabled)
...
async def run_stagehand_protocol_min() -> None:
    ...
    load_dotenv()
    args = parse_args()
    targets = selected_targets(args.site)
    if not targets:
    ...
    try:
        ...
        for target in targets:
            ...
            domain = normalize_domain(urlsplit(target.url).netloc)
            console.print(f"TARGET={target.name} DOMAIN_DETECTED={domain}")

            await session.navigate(url=target.url, page=pw_page)
            await sleep_ms(1500)
            await accept_cookies(session, pw_page, model_opts)
            ...
            for offer in offers:
                console.print(offer)
            console.print(f"{target.name.upper()}_OFFERS_COUNT={len(offers)}")
            console.print(f"{target.name.upper()}_COLLECT_MS={int((time.perf_counter() - target_started) * 1000)}")

        console.print(f"OFFERS_COUNT_TOTAL={total_offers}")
