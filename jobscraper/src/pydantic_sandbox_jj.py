...
JJ_FIRST_VISIBLE_OFFER_URL = (
    "https://justjoin.it/job-offer/epam-systems-python-engineering-trainee-poland-remote--python"
)
TRUTHY = {"1", "true", "yes"}
...
def infer_offer_pattern_from_url(first_url: str) -> Optional[OfferPattern]:
    ...
    if len(path_parts) >= 2:
        ...
        # Keep the second segment only when it looks structural (not a unique slug/id).
        is_slug_like = len(second) >= 18 and ("-" in second or re.search(r"\d", second) is not None)
        if not is_slug_like:
            include.append(second)
    return OfferPattern(first_url=first_url, include_tokens=include, exclude_tokens=[])
...
def matches_offer_pattern(url: str, pattern: Optional[OfferPattern]) -> bool:
    if pattern is None:
        return True
    path_parts = [p.lower() for p in (urlsplit(url).path or "").split("/") if p]
    if pattern.include_tokens and not all(token in path_parts for token in pattern.include_tokens):
        return False
    if pattern.exclude_tokens and any(token in path_parts for token in pattern.exclude_tokens):
        return False
    return True
...
async def extract_offer_urls(
    ...
    # Keep high-signal listing card URLs even if inferred pattern is too strict.
...
async def collect_offers(
    ...
    stagnation = 0
    step = 0
    inferred_pattern = infer_offer_pattern_from_url(
        env_str("STAGEHAND_FIRST_OFFER_URL", JJ_FIRST_VISIBLE_OFFER_URL)
    )
    if inferred_pattern:
    ...
    while True:
        ...
        step += 1
        if step > 24:
            console.print("STOP_REASON=safety_step_limit_24")
            break
        ...
        filtered = [
            ...
            if inferred_pattern is None or matches_offer_pattern(url, inferred_pattern)
        ]
        added_this_step = 0
        for url in filtered:
            if url not in seen:
        ...
        console.print(
            f"STEP={step} OFFERS={len(seen)} RAW={len(raw_extracted)} ADDED={added_this_step} "
            f"PAGE={normalize_page_url(pw_page.url)}"
        )
        ...
        if stagnation >= 3:
            ...
            break
        if not moved and stagnation >= 2:
            console.print("STOP_REASON=no_more_scroll")
            break

    return sorted(seen)


async def run_stagehand_protocol_min() -> None:
    started = time.perf_counter()
    load_dotenv()

    target_url = env_str("STAGEHAND_POC_URL", PROTOCOL_POC_URL)
...
def main() -> None:
    asyncio.run(run_stagehand_protocol_min())
