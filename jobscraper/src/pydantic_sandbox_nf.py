...
async def collect_offers(
    ...
    max_stagnation = 10
    inferred_pattern = infer_offer_pattern_from_url(env_str("STAGEHAND_FIRST_OFFER_URL", NF_FIRST_VISIBLE_OFFER_URL))
    if inferred_pattern:
...
async def run_stagehand_nf() -> None:
    started = time.perf_counter()
    load_dotenv()
    target_name = "nofluffjobs"
    target_url = env_str("STAGEHAND_POC_URL", NOFLUFF_POC_URL)
